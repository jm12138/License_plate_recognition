...
def parse_args():
    ...
    parser.add_argument("--det_model_dir", type=str, default='./inference/det_db')
    ...
    parser.add_argument("--rec_algorithm", type=str, default='RARE')
    parser.add_argument("--rec_model_dir", type=str, default='./inference/rec_rare')
...
def create_predictor(args, mode):
    ...
    if args.use_gpu:
        ...
        if args.enable_mkldnn and mode == "det":
...
def draw_ocr_box_txt(image, boxes, txts, scores):
    ...
    if scores!=[]:
        txt = '欢迎光临：\n%s' % txts[np.argmax(scores)]
        font_size = 50
        font = ImageFont.truetype(
            "./simfang.ttf", font_size, encoding="utf-8")
        draw_right.text(
            [200, 200], txt, fill=(0, 0, 0), font=font)
